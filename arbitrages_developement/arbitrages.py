...
def compute_gain(ask, symbol, bid, fee_percentage, ask_volume, bid_volume, amount_in_dollars=100, slippage=0.001, network_fee=0.001, order_book_depth=100):
    try:
        # Apply slippage to adjust ask and bid prices
        adjusted_ask = float(ask) * (1 + slippage)
        adjusted_bid = float(bid) * (1 - slippage)

        # Check if adjusted_ask or adjusted_bid is valid
        ...
        # Calculate net gain in dollars for a $100 investment
...
async def run():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        SYMBOLS = all_binance_symbols

    # Check if SYMBOLS is empty before running the main function
    if SYMBOLS:
        await main(EXCHANGES, SYMBOLS)
    else:
        print("No symbols to process. Exiting...")

async def main(exchanges, symbols):
    async with aiohttp.ClientSession() as session:
        prices = await fetch_prices_with_details(session, exchanges, symbols)
